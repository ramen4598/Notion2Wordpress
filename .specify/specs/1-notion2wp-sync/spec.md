...
- Notion 페이지에 포함된 대용량 이미지를 처리할 수 없는 경우: 업로드 실패 시 재시도 및 실패 보고
- Notion 또는 WordPress API의 일시적 레이트 리밋/오류: 지수적 재시도와 오류 보고
...
- **FR-001**: 시스템은 특정 Notion 데이터베이스 또는 페이지 집합을 모니터링할 수 있어야 한다.
...
- **FR-007**: 시스템은 동기화 과정에서 발생한 주요 오류를 로깅하고, 운영자가 확인할 수 있도록 요약을 제공해야 한다.
- **FR-008**: MVP 범위에서는 동일 콘텐츠의 중복 업로드를 허용한다(향후 버전에서 idempotency 제공). 
- **FR-009**: 동기화 동작은 WordPress에 새로운 글을 "draft" 상태로 올려야 한다(관리자 수동으로 게시).
- **FR-010**: 시스템은 `status`가 `writing` 상태인 페이지는 무시하고 동기화 대상에서 제외해야 한다.
- **FR-011**: 동기화 중 예상치 못한 오류 발생 시, 시스템은 생성된 WordPress 리소스(포스트/미디어)를 가능한 범위에서 원복하고 Notion 페이지의 `status`를 `error`로 변경해야 한다.
...
- **SyncJob**: 스케줄 정보, 시작/종료 시간, 결과(성공/실패), 변경된 항목 목록
...
- 인증/권한(WordPress API 키, Notion integration token, Telegram bot token)은 외부 비밀 저장소에서 관리된다고 가정한다.
